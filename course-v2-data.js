  ...
  weeks: [
    {
      ...
      tasks:["3h｜从工作代码中匿名抽取一个 handler→service→repository 调用链，标注依赖和错误边界","5h｜建立 course-lab，重写该调用链：最小接口、错误包装、context 传播、依赖注入","2h｜写 8 个表驱动测试，覆盖 not-found、timeout、invalid、dependency failure"],
    ...
    {title:"系统设计题库与双月模拟面试",stage:"专项验收",hours:"8–10h",outcome:"完成短链、秒杀、推送、订单、计数、扫码登录六题。",lecture:["流程：需求/估算→API/数据→核心→瓶颈→故障→观测","每题深入一个核心矛盾","答案必须有取舍和演进"],resources:[{name:"System Design Primer 中文",url:"https://github.com/donnemartin/system-design-primer/blob/master/README-zh-Hans.md",scope:"Approach、估算、cache、asynchronism、URL shortener"}],tasks:["3h｜六题各写一页骨架","2h｜随机抽题做 60 分钟设计","2h｜模拟追问容量/一致性/热点/降级/观测","1h｜修最弱设计"],questions:["如何估 QPS/存储/带宽？","秒杀与短链核心矛盾？","订单状态机防什么？"],gate:"六题完整；抽题能主动推进；追问≥80%。",repair:"先修需求和估算，不堆中间件。",optional:"录屏回看。"},
